...
// Добавьте эту функцию в конец файла rc5.cpp
int main(int argc, char* argv[]) {
    if (argc < 4) {
        std::cerr << "Usage: " << argv[0] << " <encrypt|decrypt> <input_file> <output_file> <key_file>" << std::endl;
        return 1;
    }
    ...
    try {
        ...
        std::vector<uint8_t> result;
        if (mode == "encrypt") {
            result = rc5.encrypt(input_data);
        } else if (mode == "decrypt") {
            result = rc5.decrypt(input_data);
        } else {
        ...
        std::cout << "Success: " << mode << " completed" << std::endl;
        return 0;
