...
int main(int argc, char* argv[]) {
    ...
    for (int i = flag_d_enum; i <= flag_na_enum; i += 1) {
        if (!strcmp(argv[1], CommandNames[i])) { 
            ...
            FILE* file_from = NULL;
            FILE* file_to = NULL;
            FILE* file_to_out = NULL;
            char sd[size_65-1] = "";

            if ((file_from = fopen(argv[2], "r+")) == NULL) {
                if (file_from != NULL) fclose(file_from);
                if (file_to != NULL) fclose(file_to);
                if (file_to_out != NULL) fclose(file_to_out);
                return 1;
            }

            if (argc == 4) {  
                if ((file_to = fopen(argv[3], "w+")) == NULL) {
                    if (file_from != NULL) fclose(file_from);
                    if (file_to != NULL) fclose(file_from);
                    if (file_to_out != NULL) fclose(file_from);
                    return 1;
                }
            } else {
                renam(argv[2], sd);
                if ((file_to_out = fopen(sd, "w+")) == NULL) {
                    if (file_from != NULL) fclose(file_from);
                    if (file_to != NULL) fclose(file_from);
                    if (file_to_out != NULL) fclose(file_from);
                    return 1;
