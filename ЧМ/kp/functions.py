...
def S_function_low(T, a, b, c, d , e, S_0):
    ...
    x = T
    # integral_1 = a*np.log(x) - b/2*x**2 +d*x**2/2+e*x**3/3
    integral_1 = a * np.log(np.abs(x)) - b / (2 * (x)) + c * (x) + d * ((x**2) / 2) + e * ((x**3) / 3)
    x = config.T_0
    # integral_2 = a*np.log(x) - b/2*x**2 +d*x**2/2+e*x**3/3
    integral_2 = a * np.log(np.abs(x)) - b / (2 * (x)) + c * (x) + d * ((x**2) / 2) + e * ((x**3) / 3)
    S = integral_1 - integral_2 + S_0 # 100/S_0
    return S
...
# def F_function_low(T, a, b, c, d , e, S_0, H_0):
#     # """ Вычисление энтропии при низких температурах температурах """
#     F = (H_function_low(T, a, b, c, d , e, H_0) - T * S_function_low(T, a, b, c, d , e, S_0)) / 8.31 / T
#     return F

# def F_function_high(T, a, b, c, d, e, S_0, H_0):
#     # """ Вычисление энтропии при высоких температурах температурах с поправкой T_check """
#     T_check = config.max_low_temperature
#     F = (F_function_high(T, a, b, c, d , e, H_0) - (T - T_check) * F_function_high(T, a, b, c, d , e, S_0)) / 8.31 / (T - T_check)
#     return F
...
def generate_plot_with_original(output_dir, name, temperatures, values, T_full, fit_low, fit_high):
    ...
    plt.plot(T_low, fit_low, color='blue', label='Низкие температуры')
    plt.plot(T_high, fit_high, color='red', label='Высокие температуры')
...
def process_substance(output_dir, name, filename, H_0, S_0):
    ...
    # generate_plot_with_original(output_dir + '/' + name, 'Ф', temperatures, H_values, T_full, F_fit_low, F_fit_high)
